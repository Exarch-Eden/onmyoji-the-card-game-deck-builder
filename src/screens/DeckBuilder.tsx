...
const DeckBuilder = () => {
    return (
        <div className="DeckBuilderContainer">
            ...
            {/* <div className="DeckBuilderRight"><Card {...CardInfoTest} /></div> */}
            <div className="DeckBuilderRight">{renderCards(mockCards)}</div>
        </div>
    );
};

const renderCards = (cardsList: CardInfo[]) => {
    return cardsList.map((curCard) => <Card {...curCard} key={curCard.name} />);
};
